...
from bitcoinrpc_jsonrpc import ServiceProxy, authproxy
...
class SatoshiDaemonManager(object):
   ...
   def setupSDM(self, pathToBitcoindExe=None, satoshiHome=None, \
      ...
      if satoshiHome is not None:
         ...
         if self.satoshiHome is None:
            self.satoshiHome = getBitcoinHomeDir()

      # If no new dir is specified, leave satoshi home if it's already set
      # Give it a default getBitcoinHomeDir() if not.
   ...
   def findBitcoind(self, extraSearchPaths=[]):
      ...
      if isWindows():
         ...
         searchPaths.extend([os.path.join(p, 'Bitcoin', 'daemon')
                             for p in possBaseDir])
         ...
         if getX64Flag():
            searchPaths.extend([os.path.join(p, 'bin/64')
                                for p in extraSearchPaths])
         else:
            searchPaths.extend([os.path.join(p, 'bin/32')
                                for p in extraSearchPaths])
         ...
         try:
            locs = subprocess.check_output(['whereis','bitcoind']).split()
            if len(locs)>1:
      ...
      if len(self.foundExe)>0 and len(extraSearchPaths)>0:
         ...
         if not foundIt:
            LOGERROR('Bitcoind could not be found '
                     'in the specified installation:')
            for p in extraSearchPaths:
               LOGERROR('   %s', p)
   ...
   def getGuardianPath(self):
      if isWindows():
         armoryInstall = os.path.dirname(__file__)
   ...
   def readBitcoinConf(self, makeIfDNE=False):
      ...
      if isWindows():
         if getOSVariant()[0].lower()=='xp':
            ...
            if getBitcoinForcePermissionsFlag():
               import win32process
               LOGINFO('Setting permissions on bitcoin.conf')
               cmd_icacls = [u'icacls',bitconf,u'/inheritance:r',u'/grant:r',
                             u'%s:F' % username_u16.value]
               kargs = {}
               kargs['shell'] = True
               kargs['creationflags'] = win32process.CREATE_NO_WINDOW
               icacls_out = subprocess.check_output(cmd_icacls, **kargs)
      ...
      self.bitconf['rpcport'] = int(self.bitconf.get('rpcport',
                                                     getBitcoinRPCPort()))
   ...
   def getSDMState(self):
      ...
      self.circBufferTime.append(time.time())
      ...
      if state=='BitcoindNotAvailable':
         if 'BitcoindInitializing' in self.circBufferState:
            LOGWARN('Overriding not-available state. '
                    'This should happen 0-5 times')
   ...
   def getSDMStateLogic(self):
      ...
      if not self.isRunningBitcoind():
         ...
         secSinceLastBlk = time.time() - latestInfo['toptime']
         blkspersec = latestInfo['blkspersec']
         if secSinceLastBlk > 4*HOUR or blkspersec==-1:
            return 'BitcoindSynchronizing'
         else:
            if blkspersec*20 > 2 and \
               not 'BitcoindReady' in self.circBufferState:
               return 'BitcoindSynchronizing'
            else:
               return 'BitcoindReady'
   ...
   def __backgroundRequestTopBlock(self):
      ...
      try:
         ...
         if len(self.last20queries)==0 or \
               (time.time()-self.last20queries[-1][0]) > 0.99:
            # This conditional guarantees last 20 queries spans at least 20s
            self.last20queries.append([time.time(), numblks])

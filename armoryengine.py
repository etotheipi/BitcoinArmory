...
class PyBtcAddress(object):
   ...
   #############################################################################
   def createFromEncryptedKeyData(self, ADDRESS_20, encrPrivKey32, IV16, \
                                                     chkSum=None, PUBLIC_KEY=None):
      # We expect both private key and IV to the right size
      assert(encrPrivKey32.getSize()==32)
      assert(IV16.getSize()==16)
      self.__init__()
      self.addrStr20     = ADDRESS_20
      ...
      if PUBLIC_KEY:
         self.binPublicKey65 = SecureBinaryData(PUBLIC_KEY)
   ...
   def createFromPlainKeyData(self, plainPrivKey, addr160=None, willBeEncr=False, \
      ...
      if not addr160:
         addr160 = convertKeyDataToAddress(PRIVATE_KEY=plainPrivKey)
   ...
   def unserialize(self, toUnpack):
      ...
      PRIVATE_KEY = chkzero(serializedData.get(BINARY_CHUNK, 32))
      chkPriv =         serializedData.get(BINARY_CHUNK,  4)
      iv      = SecureBinaryData(verifyChecksum(iv, chkIv))
      PRIVATE_KEY = SecureBinaryData(verifyChecksum(PRIVATE_KEY, chkPriv))

      # If this is SUPPOSED to contain a private key...
      if containsPrivKey:
         if PRIVATE_KEY.getSize()==0:
         ...
         if self.useEncryption:
            ...
            if self.createPrivKeyNextUnlock:
               ...
               self.createPrivKeyNextUnlock_IVandKey[1] = PRIVATE_KEY.copy()
            else:
               self.binInitVect16     = iv.copy()
               self.binPrivKey32_Encr = PRIVATE_KEY.copy()
         else:
            self.binInitVect16      = iv.copy()
            self.binPrivKey32_Plain = PRIVATE_KEY.copy()

      PUBLIC_KEY = chkzero(serializedData.get(BINARY_CHUNK, 65))
      chkPub =         serializedData.get(BINARY_CHUNK, 4)
      PUBLIC_KEY = SecureBinaryData(verifyChecksum(PUBLIC_KEY, chkPub))

      if containsPubKey:
         if not PUBLIC_KEY.getSize()==65:
            if self.binPrivKey32_Plain.getSize()==32:
               PUBLIC_KEY = CryptoAES().ComputePublicKey(self.binPrivKey32_Plain)
            else:
               raise UnserializeError, 'Checksum mismatch in PublicKey ' +\
                                       '('+hash160_to_addrStr(self.addrStr20)+')'

      self.binPublicKey65 = PUBLIC_KEY
   ...
   #############################################################################
   def createFromPrivateKey(self, PRIVATE_KEY, PUBLIC_KEY=None, skipCheck=False):
      """
      ...
      if isinstance(PRIVATE_KEY, str) and len(PRIVATE_KEY)==32:
         self.binPrivKey32_Plain = SecureBinaryData(PRIVATE_KEY)
      elif isinstance(PRIVATE_KEY, int) or isinstance(PRIVATE_KEY, long):
         binPriv = int_to_binary(PRIVATE_KEY, widthBytes=32, endOut=BIGENDIAN)
...
def getTxOutMultiSigInfo(binScript):
   ...
   while bup.getRemainingSize() > 0:
      ...
      if len(binChunk) == 20:
         ...
         addr160List.append(convertKeyDataToAddress(PUBLIC_KEY=binChunk))
...
def TxOutScriptExtractAddr160(binScript):
   ...
   if txoutType == TXOUT_SCRIPT_COINBASE:
      return convertKeyDataToAddress(PUBLIC_KEY=binScript[1:66])
...
class PyBtcWallet(object):
   ...
   ***NOTE:  I have ONLY implemented deterministic wallets, using ECDSA
   ...
   #############################################################################
   def touchAddress(self, ADDRESS_20):
      """
   ...
   #############################################################################
   def importExternalAddressData(self, PRIVATE_KEY=None, PRIVATE_CHECKSUM=None, \
                                       PUBLIC_KEY=None,  pubChk=None, \
                                       ADDRESS_20=None,  addrChk=None, \
      ...
      if not PRIVATE_KEY and not self.watchingOnly:
         LOGERROR('')
      ...
      if PRIVATE_KEY:
         ...
         if PRIVATE_CHECKSUM:
            PRIVATE_KEY = SecureBinaryData(verifyChecksum(PRIVATE_KEY.toBinStr(), PRIVATE_CHECKSUM))

         computedPubkey = CryptoECDSA().ComputePublicKey(PRIVATE_KEY)
         computedAddr20 = convertKeyDataToAddress(PUBLIC_KEY=computedPubkey)
      ...
      if PUBLIC_KEY:
         ...
         if pubChk:
            PUBLIC_KEY = SecureBinaryData(verifyChecksum(PUBLIC_KEY.toBinStr(), pubChk))

         if not computedAddr20:
            computedAddr20 = convertKeyDataToAddress(PUBLIC_KEY=PUBLIC_KEY)

      # The 20-byte address (pubkey hash160) should always be a python string
      if ADDRESS_20:
         if not isinstance(PUBLIC_KEY, str):
            ADDRESS_20 = ADDRESS_20.toBinStr()
         if addrChk:
            ADDRESS_20 = verifyChecksum(ADDRESS_20, addrChk)


      # Now a few sanity checks
      if self.addrMap.has_key(ADDRESS_20):
         LOGWARN('This address is already in your wallet!')
         return

      #if PUBLIC_KEY and not computedPubkey==PUBLIC_KEY:
         #raise ECDSA_Error, 'Private and public keys to be imported do not match!'
      #if ADDRESS_20 and not computedAddr20==ADDRESS_20:
         #raise ECDSA_Error, 'Supplied address hash does not match key data!'

      ADDRESS_20 = computedAddr20
      ...
      if self.useEncryption and PRIVATE_KEY and not self.kdfKey:
      ...
      if PRIVATE_KEY:
         ...
         newAddr = PyBtcAddress().createFromPlainKeyData( addr160=ADDRESS_20, \
                                  plainPrivKey=PRIVATE_KEY, publicKey65=computedPubkey,  \
         ...
         securePubKey = SecureBinaryData(PUBLIC_KEY)
         newAddr = PyBtcAddress().createFromPublicKeyData(securePubKey)
      else:
         newAddr = PyBtcAddress().createFromPublicKeyHash160(ADDRESS_20)

...
libpngVer    = '1.6.15'
qtMajorVer   = '5.4'
qtMinorVer   = '5.4.1'  # NB: ArmoryMac.pro must also be kept up to date!!!
                        # Possibly "sipFlags" below too.
sipVer       = '4.16.6' # NB: ArmoryMac.pro must also be kept up to date!!!
pyQtVer      = '5.4.1' # NB: When I'm upgraded, SIP usually has to be upgraded too.
webkitRev    = '175335'
appNopeVer   = '0.1.0'
...
#pypath_txt_template=""" PYTHON_INCLUDE=%s/include/python3.4/ PYTHON_LIB=%s/lib/python3.4/config/libpython3.4.a PYVER=python3.4"""
...
distfiles.append( [ "Qt", \
                    "qt-everywhere-opensource-src-%s.tar.gz" % qtMinorVer, \
                    "http://download.qt-project.org/official_releases/qt/%s/%s/single/qt-everywhere-opensource-src-%s.tar.gz" % (qtMajorVer, qtMinorVer, qtMinorVer), \
                    "e696b353a80ad53bcfd9535e744b5cd3246f5fd1" ] )
...
distfiles.append( [ "sip", \
                    ...
                    '073fd4c48b1ef6a574a67a92782c99f550120e23' ] )
...
distfiles.append( [ "pyqt", \
                    "PyQt-gpl-%s.tar.gz" % pyQtVer, \
                    "http://downloads.sf.net/project/pyqt/PyQt5/PyQt-%s/PyQt-gpl-%s.tar.gz" % (pyQtVer, pyQtVer), \
                    '0662379b66da422dfcbb0ca46df9d41208e859f4' ] )
...
def compile_qt():
   ...
   # NB: WebKit Will be deprecated in Qt 5.5. Will then need to use WebEngine.
   webkita = tarfilesToDL['Webkit-for-Qt']
   src = path.join(DLDIR, webkita)
   dst = path.join(qtBuildDir, 'qtwebkit/WebKitLibraries', webkita)
   copyfile(src, dst)
...
#               cwd=qtBuildDir)
   ...
   command += '-skip qtconnectivity -skip qtdeclarative -skip qtdoc '
   command += '-skip qtenginio -skip qtgraphicaleffects -skip qtimageformats '
   command += '-skip qtlocation -skip qtmultimedia -skip qtquick1 '
   command += '-skip qtquickcontrols -skip qtscript -skip qtsensors '
...
def install_qt():
   ...
   #   raise RuntimeError('Using precompiled Qt is not supported yet.')
      ...
      for mod in ['QtCore', 'QtGui', 'QtWidgets', 'QtNetwork', 'QtMacExtras']:
      ...
      # Qt 5 doesn't write this file when installed, so let's force a write.
      ...
      os.environ['QMAKESPEC'] = path.join(os.environ['QTDIR'], 'mkspecs/macx-clang')
...
def compile_pyqt():
   ...
   pyrccPath = path.join(UNPACKDIR, 'PyQt-gpl-%s/pyrcc' % pyQtVer)
...
def compile_objc_library():
   ...
   # FIX - REDO.
   sipFlags = '-w -x VendorID -t WS_MACX -t Qt_5_4_1 -x Py_v3 -B Qt_6_0_0 -o ' \
              '-P -c . -I ../workspace/unpackandbuild/PyQt-gpl-%s/sip' % pyQtVer
...
#              '-P -g -c . -I ../workspace/unpackandbuild/PyQt-mac-gpl-%s/sip' % pyQtVer
   ...
   execAndWait('../workspace/unpackandbuild/qt-everywhere-opensource-src-%s/qtbase/bin/qmake ArmoryMac.pro' % qtMinorVer, cwd=OBJCDIR)

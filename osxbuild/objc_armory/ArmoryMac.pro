...
# One day, we'll need to use static linking. Look into create_prl & similar.
...
INCLUDEPATH = $$QT_UNPACK_BASE/mkspecs/macx-clang . $$QT_UNPACK_BASE/qtbase/include $$QT_UNPACK_BASE/qtbase/include/QtWidgets $$QT_UNPACK_BASE/qtbase/include/QtCore $$QT_UNPACK_BASE/qtmacextras/include $$QT_UNPACK_BASE/qtmacextras/include/QtMacExtras .rcc/release-shared .moc $$SIP_UNPACK_BASE/siplib $$PYTHON_UNPACK_BASE/Include $$PYTHON_UNPACK_BASE
QMAKE_LFLAGS += -lc++
...
# For some reason, -F flag added for qtbase libraries (QtCore & QtWidgets) but
# not for QtMacExtras, so we have to add it manually.
# Also, we must move the resultant library (.dylib) to DESTDIR and rename it
# (.so) via QMAKE_POST_LINK.
...
LIBS += -framework QtCore -framework QtWidgets -F$$QT_UNPACK_BASE/qtmacextras/lib -framework QtMacExtras -L$$QT_UNPACK_BASE/qtbase/lib -L$$QT_UNPACK_BASE/qtmacextras/lib -framework Carbon -lz -framework AppKit -framework Python

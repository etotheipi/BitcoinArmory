...
class MockWalletFile(object):
   ...
   def doFileOperation(self, *args, **kwargs):
      pass

   def addFileOperationToQueue(self, *args, **kwargs):
      pass

   def fsyncUpdates(self, *args, **kwargs):
      pass
...
def runSerUnserRoundTripTest(tself, akp):
   ...
   if akpNew.getPrivKeyAvailability()==PRIV_KEY_AVAIL.Available:
      ...
      sipaPriv = getPrivKeyByte() + sbdPriv.toBinStr() + lastByte
...
class UtilityFuncTests(unittest.TestCase):
   ...
   def setUp(self):
      useMainnet()
      self.tmpdir = tempfile.mkdtemp("armory_keypairs")
   ...
   def testSplitChildIndex(self):
      ...
      self.assertRaises(ValueError, SplitChildIndex, 2**32+1)
   ...
   def testCreateChildIndex(self):
      ...
      self.assertRaises(ValueError, CreateChildIndex, 2**31+1, True)
...
class ABEK_Tests(unittest.TestCase):
   ...
   def test_InitABEK(self):
      ...
      self.assertEqual(abek.getChildIndex(), None)
      ...
      self.assertEqual(abek.getParent(), None)
      self.assertEqual(abek.masterEkeyRef, None)

      abek.unlock('dumb')       # shouldn't do anything, no ekey
      abek.lock()               # shouldn't do anything, no ekey
      abek.checkLockTimeout()   # shouldn't do anything, no ekey
      self.assertEqual(abek.isLocked(), False)

      self.assertEqual(abek.TREELEAF, False)
      self.assertEqual(abek.getName(), 'ABEK_Generic')
      self.assertEqual(abek.getHDPath(), 'm')
      ...
      self.assertEqual(abek.getKeys(), [])  # nothing here yet
   ...
   def testSpawnABEK(self):
      ...
      childScr = childAbek.getScrAddr(True)
      self.assertEqual(scrAddr_to_script(childScr), childAbek.getScript(True))
      self.assertEqual(scrAddr_to_addrStr(childScr), childAbek.getAddrStr())
      self.assertEqual(abek.getChildByScrAddr(childScr), childAbek)
      ...
      self.assertEqual(childAbek.getChildIndex(), nextIdx)
      ...
      ch256  = hash256(subCh.getScrAddr(True))
      ...
      self.assertEqual(childAbek.getUniqueIDBin(True), uidBin)
      self.assertEqual(childAbek.uniqueIDB58, uidB58)
      self.assertEqual(childAbek.getHDPath(), "m/%s" % ChildIndexToStr(nextIdx))
      self.assertEqual(childAbek.getAddrLocatorString(baseID="")[:4], 'A2.0')
      self.assertEqual(childAbek.getFingerprint().encode('hex'), 'bef5a2f9')

      fileName = "tmp.x"
      childAbek.writeXpubFile(fileName)
      data = open(fileName).read()
      self.assertEqual(len(data), 121)
      childAbek.writeXprvFile(fileName)
      data = open(fileName).read()
      self.assertEqual(len(data), 121)
      ...
      sipaPriv = getPrivKeyByte() + sbdPriv.toBinStr() + '\x01'
      sipaPriv = binary_to_base58(sipaPriv + computeChecksum(sipaPriv))
      self.assertEqual(abek.getSerializedPrivKey('bin'),
                       sbdPriv.toBinStr()+'\x01')
      self.assertEqual(abek.getSerializedPrivKey('base58'), sipaPriv)
   ...
   def test_InitABEK2(self):
      ...
      child256  = hash256(childAbek.getScrAddr(True))
   ...
   def testGetParentList(self):
      ...
      # Test getRoot
      self.assertEqual(abekSeed.getRoot(), abekSeed)
      self.assertEqual(abekSeed.akpChildByIndex.values()[0].getRoot(), abekSeed)
      self.assertEqual(abekSeed.akpChildByIndex.values()[0].\
            akpChildByIndex.values()[0].getRoot(), abekSeed)
      leafNode = abekSeed.akpChildByIndex.values()[0].\
            akpChildByIndex.values()[0].\
            akpChildByIndex.values()[0]
      self.assertEqual(leafNode.getRoot(), abekSeed)
      # This is the first address of the external chain of the wallet
      expectedIndexVector = [0, 0, 2147483648L]
      self.assertEqual(leafNode.getIndexVector(), expectedIndexVector)
      
      changeLeafNode = abekSeed.akpChildByIndex.values()[0].\
            akpChildByIndex.values()[1].\
            akpChildByIndex.values()[0]
      expectedChangeIndexVector = [0, 1, 2147483648L]
      self.assertEqual(changeLeafNode.getIndexVector(), expectedChangeIndexVector)
            
      # Test isChange
      # Non change is on the first chain in a wallet (index 0)
      # Change is on the second chain in a wallet (index 1)
      self.assertFalse(leafNode.isChange())
      self.assertTrue(changeLeafNode.isChange())

      # Test isImported
      # Can only test for False until Armory has BIP 32 imported addresses 
      self.assertFalse(leafNode.isImported())
      self.assertFalse(changeLeafNode.isImported())
   ...
   def testSpawnABEK_Crypt(self):
      ...
      abek.lock()
      self.assertTrue(abek.isLocked())
      self.assertEqual(abek.getPrivKeyAvailability(), PRIV_KEY_AVAIL.NeedDecrypt)
      abek.unlock(self.password, timeout=1)
      self.assertEqual(abek.getPrivKeyAvailability(), PRIV_KEY_AVAIL.Available)
      time.sleep(2)
      checkLockTimeoutTime = abek.checkLockTimeout()
      self.assertEqual(abek.getPrivKeyAvailability(), PRIV_KEY_AVAIL.NeedDecrypt)
...
class Armory135Tests(unittest.TestCase):
   ...
   def testInitA135(self):
      ...
      self.assertEqual(a135.getParent(), None)
      ...
      self.assertEqual(a135.getChildIndex(), 0)
   ...
   def testInitFromSeed32(self):
      ...
      self.assertEqual(a135rt.getUniqueIDB58(forceRecompute=True), self.rootID)
   ...
   def testInitFromSeed64(self):
      ...
      self.assertEqual(a135rt.getUniqueIDB58(forceRecompute=True), '2tyiWrmKd') # diff chain->diff id
   ...
   def testSpawnA135(self):
      ...
      for WO in [False, True]:
         ...
         while kidx+1 in self.keyList:
            ...
            self.assertEqual(a135.getParent().akpChildByIndex[0].getScrAddr(), expectScrAddr)
         ...
         for idx,a135 in a135rt.root135ChainMap.iteritems():
            ...
            if idx>0:
               self.assertEqual(a135.getParent().getScrAddr(), self.keyList[idx-1]['ScrAddr'])
               self.assertEqual(a135.getParent().akpChildByIndex[0].getScrAddr(), a135.getScrAddr())
               self.assertEqual(len(a135.getParent().akpChildByIndex), 1)
               self.assertEqual(len(a135.getParent().akpChildByScrAddr), 1)
   ...
   def test135KeyPool(self):
      for WO in [False, True]:
         ...
         for i,ch in a135rt.root135ChainMap.iteritems():
            ...
            self.assertEqual(ch.getChildIndex(), 0)
            ...
            if i>0:
               ...
               self.assertEqual(ch.getParent().akpChildByIndex[0].getScrAddr(), expectScrAddr)
...
class Imported_NoCrypt_Tests(unittest.TestCase):
   ...
   def testImportedInit(self):
      ...
      for i in range(2):
         ...
         self.assertEqual(aikp.getParent(), None)
         ...
         self.assertEqual(aikp.getChildIndex(), None)
   ...
   def testCreateImportedRoot(self):
      ...
      self.assertEqual(airt.getChildIndex(), None)
      ...
      self.assertEqual(aikp.getParent().getScrAddr(), airt.getScrAddr())
...
class BIP32TestVectorTests(unittest.TestCase):
   """
   Test the vectors from the bip-0032 proposal:
   ...
   def testVector1(self):
      parent = None
      for i, vector in enumerate(BIP32TestVectors):
         if parent:
            current = parent.spawnChild(BIP32TestVectors[i-1]['nextChild'])
         else:
            # root
            current = ArmoryBip32Seed()
            current.initializeFromSeed(vector['seed'], fillPool=False)
         self.assertEqual(current.getExtendedPubKey(), vector['xpub'])
         self.assertEqual(current.getExtendedPrivKey(), vector['xprv'])
         parent = current

   def testVector2(self):
      parent = None
      for i, vector in enumerate(BIP32TestVectors2):
         if parent:
            current = parent.spawnChild(BIP32TestVectors2[i-1]['nextChild'])
         else:
            # root
            current = ArmoryBip32Seed()
            current.initializeFromSeed(vector['seed'], fillPool=False)

      ...
      try:
         while True:
            ...
            if 2<t<3 or 6<t<7:
               ...
               print 'LENGTH OF NOTIFY FILE:', (len(d) if d else 0)
               print '*****'
               print 'took %0.6f seconds' % (time.time() - s)

            if 9<t<10 or 12<t<13:
               s = time.time()
               d = adf.getAnnounceFile('notify', forceCheck=True) 
               print '*****'
               print 'LENGTH OF NOTIFY FILE:', (len(d) if d else 0)
...
class parseChangelogTest(unittest.TestCase):
   ...
   def tearDown(self):
      removeIfExists(self.TEST_FILE)

   def testInit(self):
      result = changelogParser(filename=None, filetext=None)
      self.assertEqual(result.changelog, [])
      f = open(self.TEST_FILE, 'w')
      f.write(changelogTestText)
      f.close()
      result = changelogParser(filename=self.TEST_FILE)
      self.assertEqual(self.clh.changelog, result.changelog)


   def testParse(self):
      self.assertEqual(self.clh.parseChangelogText(None), None)
      self.assertEqual(self.clh.parseChangelogText(SIGNED_BLOCK_HEAD), None)
      text = 'VERSION 1\nRELEASED 1/1/01\n- doing something cool\nsomething'
      self.assertEqual(self.clh.parseChangelogText(text)[0][:2], ['1','1/1/01'])
      result = self.clh.getChangelog(dontStartBefore=0)
      self.assertEqual(result, [])
      result = self.clh.getChangelog(stopAtVersion=10000000)
      self.assertEqual(result, [])
...
class parseDownloadTest(unittest.TestCase):
   ...
   def testInit(self):
      result = downloadLinkParser(filename=None, filetext=None)
      self.assertEqual(result.downloadMap, {})
      f = open(self.TEST_FILE, 'w')
      f.write(downloadTestText)
      f.close()
      result = downloadLinkParser(filename=self.TEST_FILE)
      self.assertEqual(self.dl.downloadMap, result.downloadMap)


   def testParse(self):
      result = self.dl.parseDownloadList(None)
      self.assertEqual(result, {})
      # this should cause an error, which gives back None
      result = self.dl.parseDownloadList(SIGNED_BLOCK_HEAD)
   ...
   def testParseDL(self):
      ...
      dllink = self.dl.getDownloadLink()
      self.assertEqual(dllink, None)
      self.dl.downloadMap = 'nonsense'
      dllink = self.dl.getDownloadLink('nonsense')
      self.assertEqual(dllink, None)



   def testPrintDownloadMap(self):
      result = self.dl.printDownloadMap()
      self.assertTrue("http://url/armory_0.91.exe" in result)


################################################################################
class parseNotifyTest(unittest.TestCase):

   TEST_FILE = 'notification.log'
   ...
   def testInit(self):
      result = notificationParser(filename=None, filetext=None)
      self.assertEqual(result.notifications, {})
      f = open(self.TEST_FILE, 'w')
      f.write(notifyTestText)
      f.close()
      result = notificationParser(filename=self.TEST_FILE)
      self.assertEqual(self.notify.notifications, result.notifications)


   def testParse(self):
      result = self.notify.parseNotificationText(None)
      self.assertEqual(result, None)
      # this should cause an error, which gives back None
      result = self.notify.parseNotificationText(SIGNED_BLOCK_HEAD)
      self.assertEqual(result, None)

...
class MockLockbox(object):
   def __init__(self, lboxID, lboxName, p2shScrAddr, M, N):
      self.uniqueIDB58 = lboxID
      self.shortName   = lboxName
      self.p2shScrAddr = p2shScrAddr
      self.M           = M
      self.N           = N
...
class ScriptToDispStrTest(unittest.TestCase):
   ...
   def setUp(self):
      ...
      # Some scripts that we know will be unknown
      self.binScriptP2PK_Unk = pubkey_to_p2pk_script(self.makePubKey('\x3a'))
      self.binScriptP2PKH_Unk = hash160_to_p2pkhash_script('\xa8'*20)
      self.binScriptP2SH_Unk = script_to_p2sh_script(self.binScriptNonStd)
      ...
      self.binScriptTestList = [ \
               ...
               [self.binScriptP2PKH_Unk,'1GNnZrBi2tFJ71BncjeHxNrJ6dLQ5SaRyc'],
               [self.binScriptP2PK_Unk, '1JVHcmRnXjsDDcUdwqfc1gfCExuETBXPqE'],
               [self.binScriptP2SH_Unk, '3NgtiSVM5yiGfnGao3WEVqoairJTVeiXt5'] ]
         ...
         for scr,addrStr in binScrList:
            ...
            for pref in [True, False]:
               ...
               for lenMax in lenList:
                  print '      ',getDisplayStringForScript(scr, wltMap, lboxList, pref, lenMax)

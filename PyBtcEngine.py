...
##### STR/HASHDIGEST
def str_to_hashDigest(s):
   return sha256(sha256(s).digest()).digest()

##### STR/ADDRESSDIGEST
def str_to_addrDigest(s):
...
def address_to_key (s):
   # strip off leading '1'
   s = ('%x' % base58Str_to_addrStr(s[1:])).decode ('hex_codec')
   hash160, check0 = s[:-4], s[-4:]
   check1 = str_to_hashdigest('\x00' + hash160)[:4]
   if check0 != check1:
      raise BadAddress (s)
   return hash160
...
class wallet:
   ...
   def new_key (self):
      ...
      file.close()
      pubkey = k.get_pubkey()
      addr = key_to_address (str_to_addrdigest(pubkey))

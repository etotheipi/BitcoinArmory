...
def execAndWait(cli_str):
   print '*** Executing:', cli_str[:400], '...'
   process = Popen(cli_str, shell=True, stdout=PIPE, stderr=STDOUT)

   # Output from Popen needs an extra nudge to get to the user.
   while True:
...
execAndWait('%s export DEBFULLNAME="Armory Technologies, Inc."; yes 2>/dev/null | head -1 | dh_make -s -e support@bitcoinarmory.com -f ../%s.tar.gz' % (faketimeVars, pkgdir))
for f in dpkgfiles:
